...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: order, error: ordErr } = await admin.from('orders').insert({
      ...
      shipping_service_id: shipping?.service_id || null,
      ship_weight_g: shipping?.package?.weight_g || null,
      ship_length_cm: shipping?.package?.length_cm || null,
      ship_width_cm: shipping?.package?.width_cm || null,
      ship_height_cm: shipping?.package?.height_cm || null,
